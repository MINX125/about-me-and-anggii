...
function initEnvelope() {
  ...
  function openEnvelope() {
    ...
    spawnHeartsBurst(rect.left + rect.width / 2, rect.top + 80, 14);

    // Smoothly ensure letter is centered in view on mobile devices
    setTimeout(() => {
      if (window.innerWidth <= 768 && letterSection) {
        const topPos = envelope.getBoundingClientRect().top + window.pageYOffset - 80;
        window.scrollTo({
          top: topPos,
          behavior: 'smooth'
        });
